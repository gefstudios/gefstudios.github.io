var j=Object.defineProperty;var O=(e,t,a)=>t in e?j(e,t,{enumerable:!0,configurable:!0,writable:!0,value:a}):e[t]=a;var d=(e,t,a)=>O(e,typeof t!="symbol"?t+"":t,a);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function a(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function o(s){if(s.ep)return;s.ep=!0;const r=a(s);fetch(s.href,r)}})();const b=[{id:"delivery-sort",title:"Delivery Sort",shortDescription:"Um puzzle arcade frenético e estratégico onde organização rápida e raciocínio lógico são as chaves para o sucesso operacional.",longDescription:"Delivery Sort é um puzzle arcade onde cada segundo conta. Organize entregas, resolva desafios lógicos e domine mecânicas frenéticas em um universo operacional neon. O jogo já está em produção e disponível para download na Google Play.",icon:"/assets/delivery-sort-icon.png",status:"live",statusLabel:"Disponível",badge:"LIVE",storeLinks:{googlePlay:"https://play.google.com/store/apps/details?id=com.deliverysort.game"},features:["Puzzles arcade com ritmo acelerado","Progressão de fases com dificuldade crescente","Highscores via Google Play Games / Apple Game Center"],privacyPolicy:{title:"POLÍTICA DE PRIVACIDADE",subtitle:"[ DELIVERY SORT ]",updatedAt:"Junho de 2026",intro:"A Gef Studios descreve através deste documento as práticas de tratamento de dados aplicadas especificamente ao aplicativo mobile Delivery Sort.",hash:"#privacyDeliverySort",modalId:"delivery-modal",sections:[{title:"1. Coleta e Uso de Informações",content:"O jogo Delivery Sort foi projetado para focar puramente na experiência de jogabilidade. Armazenamos de forma estritamente local (no seu dispositivo) ou via serviços nativos da plataforma (Google Play Games / Apple Game Center):",list:["Pontuações máximas (Highscores) e progresso das fases de organização de entregas.","Dados analíticos anônimos de telemetria para otimização de performance e ajustes de balanceamento dos puzzles."]},{title:"2. Monetização e Publicidade",content:"Para viabilizar a distribuição gratuita, o aplicativo pode conter redes integradas de publicidade que coletam identificadores anônimos de anúncios do sistema (como o IDFA ou AAID) para fins de conformidade e relevância de exibição."},{title:"3. Direitos e Contato",content:"O usuário detém total controle sobre a limpeza de dados locais do aplicativo através das configurações padrão do sistema operacional do smartphone."}]}},{id:"next-universe-1",title:"Próximo Universo",shortDescription:"Novas ideias já estão sendo prototipadas na nossa incubadora de realidades virtuais.",status:"confidential",statusLabel:"Fase de Briefing"},{id:"next-universe-2",title:"Próximo Universo",shortDescription:"Novas ideias já estão sendo prototipadas na nossa incubadora de realidades virtuais.",status:"confidential",statusLabel:"Fase de Briefing"}];function k(e){return b.find(t=>t.id===e)}function T(e){return`#/game/${e}/baixar`}function C(e){var t,a;return!!((t=e.storeLinks)!=null&&t.googlePlay||(a=e.storeLinks)!=null&&a.appStore)}function _(){return`
    ...
    ${b.filter(t=>t.privacyPolicy).map(t=>{const a=t.privacyPolicy,o=a.sections.map(s=>`
          <div>
            <h4 class="font-gaming text-white font-bold uppercase text-md mb-2">${s.title}</h4>
            <p class="text-gray-400">${s.content}</p>
            ${s.list?`<ul class="list-disc pl-5 mt-2 space-y-1 text-gray-400">
                    ${s.list.map(r=>`<li>${r}</li>`).join("")}
                  </ul>`:""}
          </div>
        `).join("");return`
      ...
      `}).join("")}
  `}function z(){const e=document.getElementById("contact-form"),t=document.getElementById("submit-btn"),a=document.getElementById("success-message");if(!e||!t||!a)return;const o=()=>{t.disabled=!1,t.innerText="TRANSMITIR SINAL",t.classList.remove("opacity-50","cursor-not-allowed")};e.addEventListener("submit",s=>{s.preventDefault(),t.disabled=!0,t.innerText="TRANSMITINDO...",t.classList.add("opacity-50","cursor-not-allowed");const r=new FormData(e),i=Object.fromEntries(r.entries());fetch("https://api.web3forms.com/submit",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(i)}).then(async n=>{const l=await n.json();if(n.status===200){e.classList.add("hidden"),a.classList.remove("hidden"),a.classList.add("flex"),e.reset();return}console.log(l),alert(`Falha na transmissão: ${l.message}`),o()}).catch(n=>{console.log(n),alert("Erro de conexão ao transmitir o sinal."),o()})})}const m=".scroll-reveal, .reveal",g=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let c=null;function f(e){e.classList.add("is-visible"),e.classList.add("active")}function M(e){e.dataset.revealOnce==="false"&&(e.classList.remove("is-visible"),e.classList.remove("active"))}function v(e=document){c&&e.querySelectorAll(m).forEach(t=>{t.classList.contains("is-visible")||c.observe(t)})}function S(){if(g){document.querySelectorAll(m).forEach(f);return}if(c){v();return}c=new IntersectionObserver(e=>{e.forEach(t=>{const a=t.target;if(t.isIntersecting){const o=Number(a.dataset.revealDelay??0);o>0?window.setTimeout(()=>f(a),o):f(a),a.dataset.revealOnce!=="false"&&(c==null||c.unobserve(a))}else M(a)})},{root:null,rootMargin:"0px 0px -8% 0px",threshold:.12}),v()}function $(){g||document.querySelectorAll("[data-hero-intro]").forEach((e,t)=>{window.setTimeout(()=>f(e),120+t*140)})}function B(){const e=document.getElementById("scroll-progress");if(!e||g)return;let t=!1;const a=()=>{const o=document.documentElement.scrollHeight-window.innerHeight,s=o>0?window.scrollY/o*100:0;e.style.width=`${s}%`,t=!1};window.addEventListener("scroll",()=>{t||(requestAnimationFrame(a),t=!0)},{passive:!0}),a()}function A(){if(g)return;const e=document.getElementById("hero");if(!e)return;const t=document.getElementById("hero-logo-wrap"),a=document.getElementById("hero-copy"),o=document.getElementById("bg-canvas");let s=!1;const r=()=>{const i=window.scrollY,n=e.offsetHeight,l=Math.min(i/n,1);t&&(t.style.transform=`translateY(${i*.22}px) scale(${1-l*.08})`,t.style.opacity=`${1-l*.55}`),a&&(a.style.transform=`translateY(${i*.1}px)`,a.style.opacity=`${1-l*.75}`),o&&(o.style.opacity=`${.4-l*.15}`),s=!1};window.addEventListener("scroll",()=>{s||(requestAnimationFrame(r),s=!0)},{passive:!0})}function N(e=document){e.querySelectorAll(m).forEach(t=>{t.dataset.revealOnce==="false"&&t.classList.remove("is-visible","active")}),v(e)}function F(e){e.classList.remove("section-arrive"),e.offsetWidth,e.classList.add("section-arrive"),e.querySelectorAll(m).forEach((a,o)=>{a.classList.remove("is-visible","active"),window.setTimeout(()=>f(a),160+o*100)})}const I=new Set(["games","about","contact"]),G=88,R=1e3;function q(e){return 1-Math.pow(1-e,3)}function H(e,t=R){return new Promise(a=>{const o=window.scrollY,s=e-o;if(Math.abs(s)<2){a();return}const r=performance.now();function i(n){const l=n-r,p=Math.min(l/t,1);window.scrollTo(0,o+s*q(p)),p<1?requestAnimationFrame(i):a()}requestAnimationFrame(i)})}function D(e){const t=document.getElementById(e);if(!t)return;const a=t.getBoundingClientRect().top+window.scrollY-G;H(a).then(()=>{F(t)})}function Y(){document.addEventListener("click",e=>{var o;const t=e.target.closest('a[href^="#"]');if(!t)return;const a=((o=t.getAttribute("href"))==null?void 0:o.replace(/^#/,""))??"";if(I.has(a)&&document.getElementById("games")){if(e.preventDefault(),window.location.hash===`#${a}`){D(a);return}window.location.hash=a}})}function V(){const e=window.location.hash.replace(/^#/,"");I.has(e)&&document.getElementById(e)&&requestAnimationFrame(()=>{requestAnimationFrame(()=>D(e))})}function U(){document.querySelectorAll("[data-store-redirect]").forEach(e=>{e.addEventListener("click",()=>{e.classList.add("opacity-80","scale-[0.98]"),e.innerHTML=`
      ...
      `})})}function X(){S(),B(),A(),$()}function J(e=document){N(e),$(),A()}function W(){S()}function Q(){document.querySelectorAll(".card-3d").forEach(e=>{e.addEventListener("mousemove",t=>{const a=e.getBoundingClientRect(),o=t.clientX-a.left,s=t.clientY-a.top,r=a.width/2,n=(a.height/2-s)/12,l=(o-r)/12;e.style.transform=`rotateX(${n}deg) rotateY(${l}deg) scale(1.01)`}),e.style.perspective="1000px",e.style.transition="transform 0.1s ease-out",e.addEventListener("mouseleave",()=>{e.style.transform="rotateX(0deg) rotateY(0deg) scale(1)",e.style.transition="transform 0.5s ease"})})}function K(){const e=document.querySelector(".carousel-container");if(!e)return;let t=!1,a=0,o=0;e.addEventListener("mousedown",s=>{t=!0,a=s.pageX-e.offsetLeft,o=e.scrollLeft,e.style.scrollSnapType="none"}),e.addEventListener("mouseleave",()=>{t=!1}),e.addEventListener("mouseup",()=>{t=!1,e.style.scrollSnapType="x mandatory"}),e.addEventListener("mousemove",s=>{if(!t)return;s.preventDefault();const i=(s.pageX-e.offsetLeft-a)*1.5;e.scrollLeft=o-i})}const Z=["#privacy","#privacyDeliverySort"];function u(e,t){const a=document.getElementById(e);if(a){if(t){a.classList.remove("hidden"),a.classList.add("flex"),setTimeout(()=>{a.classList.remove("opacity-0"),a.classList.add("opacity-100")},10),document.body.style.overflow="hidden";return}a.classList.remove("opacity-100"),a.classList.add("opacity-0"),setTimeout(()=>{a.classList.remove("flex"),a.classList.add("hidden")},300),document.body.style.overflow="",Z.includes(window.location.hash)&&history.replaceState(null,"",window.location.pathname+window.location.search)}}function ee(){document.addEventListener("click",e=>{const t=e.target,a=t.closest("[data-modal]"),o=t.closest("[data-close-modal]");if(a){e.preventDefault();const s=a.dataset.modal;s&&u(s,!0);return}if(o){const s=o.dataset.closeModal;s&&u(s,!1)}})}function P(){const e=window.location.hash;if(e==="#privacy"){u("general-modal",!0);return}e==="#privacyDeliverySort"&&u("delivery-modal",!0)}function y(){return`
  ...
  `}function te(e){var i,n;const t=e.features?`
      <div class="space-y-4">
        <h3 class="font-gaming text-xl text-[#00f3ff] uppercase tracking-wider">Destaques</h3>
        <ul class="space-y-2 text-gray-400">
          ${e.features.map(l=>`<li class="flex items-start gap-3"><span class="text-[#9d4edd] mt-1">▸</span><span>${l}</span></li>`).join("")}
        </ul>
      </div>
    `:"",a=C(e)?[(i=e.storeLinks)!=null&&i.googlePlay||(n=e.storeLinks)!=null&&n.appStore?`<a href="${T(e.id)}" class="font-gaming bg-[#00f3ff] text-black px-6 py-3 uppercase text-sm tracking-widest hover:bg-white transition-all duration-300 shadow-[0_0_15px_rgba(0,243,255,0.25)]">
              Baixar Grátis
            </a>`:""].filter(Boolean).join(""):"",o=a?`<div class="flex flex-wrap gap-4 pt-2">${a}</div>`:"",s=e.privacyPolicy?`<a href="${e.privacyPolicy.hash}" data-modal="${e.privacyPolicy.modalId}" class="font-gaming border border-gray-800 text-gray-400 hover:text-[#00f3ff] hover:border-[#00f3ff] px-6 py-3 uppercase text-sm tracking-widest transition-all duration-300">
    ...
    <section class="min-h-screen pt-28 pb-20 px-6 max-w-5xl mx-auto">
      ...
      <div class="grid md:grid-cols-2 gap-12 items-start reveal">
        <div class="bg-[#121214] border border-gray-800 p-6 relative">
          ${e.badge?`<span class="absolute top-4 right-4 font-gaming text-xs bg-[#00f3ff] text-black font-black px-2 py-1 tracking-wider shadow-[0_0_10px_var(--neon-cyan)]">${e.badge}</span>`:""}
          ${r}
        </div>
  ...
  `}function ae(e){var r,i,n,l;const t=((r=e.storeLinks)==null?void 0:r.googlePlay)??((i=e.storeLinks)==null?void 0:i.appStore);if(!t)return"";const a=(n=e.storeLinks)!=null&&n.googlePlay?"Google Play Store":"App Store",o=(l=e.storeLinks)!=null&&l.googlePlay?"Google Play":"App Store",s=e.features?e.features.map(p=>`
    ...
    <section class="min-h-screen pt-28 pb-20 px-6 flex items-center justify-center">
      <div class="w-full max-w-lg reveal">
        ...
        <div class="relative bg-[#121214] border border-[#00f3ff]/30 p-8 md:p-10 text-center shadow-[0_0_40px_rgba(0,243,255,0.12)]">
          ...
          ${s?`<ul class="text-left space-y-2 mb-8 border border-gray-800 bg-[#0a0a0c]/60 p-5">${s}</ul>`:""}
  ...
  `}function oe(e,t=0){if(e.status==="confidential"||e.status==="briefing")return re(e,t);const a=e.privacyPolicy?`<a href="${e.privacyPolicy.hash}" data-modal="${e.privacyPolicy.modalId}" class="text-xs font-gaming font-bold bg-gray-900 border border-gray-800 text-gray-400 hover:text-[#00f3ff] hover:border-[#00f3ff] px-3 py-1 uppercase tracking-wider transition-all duration-300">
        Privacidade
      </a>`:"",o=e.badge?`<span class="absolute top-0 right-0 font-gaming text-xs bg-[#00f3ff] text-black font-black px-2 py-1 tracking-wider z-20 shadow-[0_0_10px_var(--neon-cyan)]">${e.badge}</span>`:"";return`
  ...
  `}function ie(){const e=b.map((t,a)=>oe(t,a)).join("");return`
    ${w()}
  ...
  `}const ne={"/":()=>ie()},le=new Set(["games","about","contact","privacy","privacyDeliverySort"]);function ce(){const e=window.location.hash.replace(/^#/,"");if(!e||e==="/")return{path:"/",params:{},isAnchorOnly:!1};const t=e.split("/").filter(Boolean);return t[0]==="game"&&t[1]&&t[2]==="baixar"?{path:"/game/:id/baixar",params:{id:t[1]},isAnchorOnly:!1}:t[0]==="game"&&t[1]?{path:"/game/:id",params:{id:t[1]},isAnchorOnly:!1}:le.has(t[0])&&t.length===1?{path:"/",params:{},isAnchorOnly:!0}:{path:"/",params:{},isAnchorOnly:!1}}function h(e=!1){window.scrollTo({top:0,behavior:e?"instant":"smooth"})}function L(){var r,i;const e=document.getElementById("app");if(!e)return;const{path:t,params:a,isAnchorOnly:o}=ce(),s=e.querySelector("#games")!==null;if(t==="/game/:id/baixar"){const n=k(a.id),l=n&&(((r=n.storeLinks)==null?void 0:r.googlePlay)||((i=n.storeLinks)==null?void 0:i.appStore));e.innerHTML=l?ae(n):E(),requestAnimationFrame(()=>h(!0))}else if(t==="/game/:id"){const n=k(a.id);e.innerHTML=n?te(n):E(),requestAnimationFrame(()=>h(!0))}else(!o||!s)&&(e.innerHTML=ne["/"]());e.insertAdjacentHTML("beforeend",_()),de(),P(),V()}function E(){return`
  ...
  `}function de(){W(),Q(),K(),z(),U(),J()}function fe(){Y(),document.addEventListener("click",e=>{e.target.closest("a[data-scroll-top]")&&h()}),window.addEventListener("hashchange",L),window.addEventListener("hashchange",P),L()}class pe{constructor(t){d(this,"x");d(this,"y");d(this,"speedX");d(this,"speedY");d(this,"size");this.canvas=t,this.x=Math.random()*t.width,this.y=Math.random()*t.height,this.speedX=(Math.random()-.5)*.6,this.speedY=(Math.random()-.5)*.6,this.size=Math.random()*2+1}update(){this.x+=this.speedX,this.y+=this.speedY,this.x>this.canvas.width&&(this.x=0),this.x<0&&(this.x=this.canvas.width),this.y>this.canvas.height&&(this.y=0),this.y<0&&(this.y=this.canvas.height)}draw(t){t.fillStyle="#00f3ff",t.beginPath(),t.arc(this.x,this.y,this.size,0,Math.PI*2),t.fill()}}let x=null;function ue(){const e=document.getElementById("bg-canvas");if(!e)return;const t=e.getContext("2d");if(!t)return;x!==null&&cancelAnimationFrame(x);const a=[],o=60,s=()=>{e.width=window.innerWidth,e.height=window.innerHeight};s(),window.addEventListener("resize",s);for(let i=0;i<o;i++)a.push(new pe(e));const r=()=>{t.clearRect(0,0,e.width,e.height),a.forEach(i=>{i.update(),i.draw(t)}),x=requestAnimationFrame(r)};r()}ue();ee();X();fe();
